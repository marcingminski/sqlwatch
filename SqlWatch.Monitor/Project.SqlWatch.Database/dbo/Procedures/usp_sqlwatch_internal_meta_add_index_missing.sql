CREATE PROCEDURE [dbo].[usp_sqlwatch_internal_meta_add_index_missing]
	@xdoc int,
...
begin
	...
	select
		...
		inner join [dbo].[sqlwatch_meta_database] db
			...
			and db.[database_create_date] = idx.[database_create_date]
		...
		where ed.[snapshot_type_id] is null
			and (
				idx.[equality_columns] is not null
				or idx.[inequality_columns] is not null
				or idx.[included_columns] is not null
				)
	...
	when not matched by target then
		...
		values (
				...
				, source.[sqlwatch_table_id]
				, source.[equality_columns] 
				, source.[inequality_columns] 
