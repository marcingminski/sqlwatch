...
'"/* SQLWATCH get remote instance ' + T.TABLE_SCHEMA + '.' + TABLE_NAME + ' */' + case when T.TABLE_NAME <> 'sqlwatch_logger_snapshot_header' then '
...
select ' + stuff ((
	...
	, sql_expression_destination_keys_type_1 = '"/* SQLWATCH get repository keys ' + T.TABLE_SCHEMA + '.' + TABLE_NAME + ' */' + case when T.TABLE_NAME <> 'sqlwatch_logger_snapshot_header' then '
declare @snapshot_type_id tinyint, @snapshot_time datetime
select top 1 @snapshot_type_id = snapshot_type_id from ' + T.TABLE_SCHEMA + '.' + T.TABLE_NAME + ' (nolock)
select @snapshot_time = snapshot_time from ##sqlwatch_logger_snapshot_header_last
where snapshot_type_id = @snapshot_type_id
